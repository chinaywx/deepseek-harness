...
describe('computeColumns', () => {
  ...
  it('step 2: details shrinks first, center pinned at min', () => {
    // SIDEBAR_DEFAULT + DETAILS_DEFAULT + CENTER_MIN > 1250; details concedes to 1250 - SIDEBAR_DEFAULT - CENTER_MIN.
    const cols = computeColumns(1250, open(SIDEBAR_DEFAULT), open(DETAILS_DEFAULT))
    expect(cols).toEqual({ sidebar: SIDEBAR_DEFAULT, center: CENTER_MIN, details: 1250 - SIDEBAR_DEFAULT - CENTER_MIN })
  })
  ...
  it('step 3: details auto-closes when its min still starves center — sidebar holds its preference', () => {
    // SIDEBAR_DEFAULT + DETAILS_MIN + CENTER_MIN just exceeds viewport → details 0; sidebar untouched.
    const cols = computeColumns(SIDEBAR_DEFAULT + DETAILS_MIN + CENTER_MIN - 1, open(SIDEBAR_DEFAULT), open(DETAILS_DEFAULT))
    expect(cols).toEqual({ sidebar: SIDEBAR_DEFAULT, center: DETAILS_MIN + CENTER_MIN - 1, details: 0 })
  })
  ...
  it('recovery is pure: re-widening restores preferred widths untouched', () => {
    const squeezed = computeColumns(SIDEBAR_DEFAULT + DETAILS_MIN + CENTER_MIN - 1, open(SIDEBAR_DEFAULT), open(DETAILS_DEFAULT))
